...
class CPage extends CHtmlBlock
{
	function parseBlock(&$html)
	{
        $pid = ipar('id');
        if ($pid > 0) {
            $post = CBlogsTools::getPostById($pid);
        }
        ...
        $pagerOnPage = 20;
        $pagerUrl = g('path','url_administration') . 'blogs_post_comments.php?id=' . $pid . '&p=%s';

        $itemsTotal = CBlogsTools::countCommentsByPostId($pid);
        $items = CBlogsTools::getCommentsByPostIdAdmin($pid, (($page - 1) * $pagerOnPage) . ',' . $pagerOnPage);
...
$page = new CPage("main", $g['tmpl']['dir_tmpl_administration'] . "blogs_post_comments.html");
